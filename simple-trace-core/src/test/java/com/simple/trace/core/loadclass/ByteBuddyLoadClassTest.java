...
public class ByteBuddyLoadClassTest {
     ...
     * INJECTION 不可再manifest中使用，并且不可使用 ClassLoader::getResourceAsStream方法访问
     *
    ...
    public void modifyFunctionReturnValue() throws InstantiationException, IllegalAccessException, IOException {
        DynamicType.Unloaded<PrintSomething> unloaded = new ByteBuddy().subclass(PrintSomething.class).method(ElementMatchers.named("printFirst")).intercept(FixedValue.value("none argument")).method(ElementMatchers.named("printSecond")).intercept(FixedValue.value("one argument")).make();
        //unloaded.saveIn(new File("a"));
        Class<? extends PrintSomething> aClass = unloaded.load(getClass().getClassLoader(), ClassLoadingStrategy.Default.WRAPPER).getLoaded();
    ...
    public void interceptWithStaticClass() throws InstantiationException, IllegalAccessException, IOException {
        DynamicType.Unloaded<PrintSomething> unloaded = new ByteBuddy().subclass(PrintSomething.class).method(ElementMatchers.named("printSecond")).intercept(MethodDelegation.to(PrintSomethingInterceptor.class)).make();
        //unloaded.saveIn(new File("a"));
        DynamicType.Loaded<PrintSomething> loaded = unloaded.load(getClass().getClassLoader(), ClassLoadingStrategy.Default.WRAPPER);
    ...
    public void interceptWithObject() throws InstantiationException, IllegalAccessException, IOException {
        DynamicType.Unloaded<PrintSomething> unloaded = new ByteBuddy().subclass(PrintSomething.class).method(ElementMatchers.named("printSecond")).intercept(MethodDelegation.to(new PrintSomethingInterceptorS())).make();
        //unloaded.saveIn(new File("a"));
        DynamicType.Loaded<PrintSomething> loaded = unloaded.load(getClass().getClassLoader(), ClassLoadingStrategy.Default.WRAPPER);
        Class<? extends PrintSomething> aClass = loaded.getLoaded();
        PrintSomething newInstance = aClass.newInstance();
